...
export async function POST(req: NextRequest) {
  const data = readData();
  const { nama, jenis_default, kontak } = await req.json();
  if (!nama) return NextResponse.json({ error: "Nama wajib diisi" }, { status: 400 });
  const now = new Date().toISOString();
  const id = nextId(data, "vendors");
  data.vendors.push({ id, nama, jenis_default: jenis_default || "", kontak: kontak || "", aktif: true, created_at: now });
  writeData(data);
...
export async function PATCH(req: NextRequest) {
  const data = readData();
  const { id, nama, jenis_default, kontak, aktif } = await req.json();

...
export default function PortalPage({ params }) {
  ...
  const submitLapor = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/produksi-transfer", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({po_id:showModal.po_id,dari:"washing",ke:"gudang",vendor_id:id,jumlah_kirim:total,sizeBreakdown:items,catatan}) });
  ...
  return (
    <div style={{position:"fixed",inset:0,overflowY:"auto",background:"#0F172A"}}>
      <div style={{background:"#1E293B",padding:"24px 28px",borderBottom:"1px solid #334155",display:"flex",alignItems:"center",gap:16}}>
        <div style={{background:"#06B6D4",width:48,height:48,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <Droplets size={24} color="white" />
        </div>
        <div>
          <h1 style={{margin:0,color:"white",fontSize:20,fontWeight:800}}>Portal Washing &mdash; {vendor?.nama||"..."}</h1>
      ...
      <div style={{padding:28}}>
        {aktif.length===0 ? (
          <div style={{background:"#1E293B",borderRadius:16,padding:40,textAlign:"center"}}>
            <div style={{color:"#64748B",fontWeight:700}}>Belum ada pakaian yang dikirim ke tahap Washing</div>
          </div>
        ) : aktif.map(t => (
          <div key={t.id} style={{background:"#1E293B",borderRadius:16,padding:20,marginBottom:12,border:"1px solid #334155"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>openLapor(t)} style={{background:"#06B6D4",color:"white",padding:"10px 18px",borderRadius:10,border:"none",fontWeight:800,cursor:"pointer",fontSize:13}}>Lapor Selesai</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
          <div style={{background:"#1E293B",borderRadius:20,width:"100%",maxWidth:480,border:"1px solid #334155"}}>
            ...
            <form onSubmit={submitLapor} style={{padding:24}}>
              ...
              <button type="submit" disabled={submitting} style={{width:"100%",background:"#06B6D4",color:"white",padding:15,borderRadius:12,border:"none",fontWeight:800,fontSize:15,cursor:"pointer"}}>{submitting?"Mengirim...":"Kirim Laporan ke Gudang"}</button>
            </form>
          </div>
        </div>
      )}

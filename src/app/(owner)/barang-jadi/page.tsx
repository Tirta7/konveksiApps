    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#F1F5F9", overflow: "hidden", position: "relative" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 32px" }}>
          {loading ? (
            <div style={{ padding: 40, textAlign: "center", color: "#94A3B8", fontWeight: 600 }}>⏳ Memuat bundle...</div>
          ) : filtered.length === 0 ? (
            <div style={{ padding: 60, textAlign: "center", background: "white", borderRadius: 20, border: "1px solid #E2E8F0" }}>
              ...
              <div style={{ fontSize: 13, color: "#64748B" }}>Belum ada bundle yang selesai diproduksi.</div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, maxWidth: 1000, margin: "0 auto" }}>
              {filtered.map((b: any) => {
                ...
                return (
                  <div key={b.id} className="batch-item-hover" onClick={() => setSelected(b)} style={{ background: "white", border: isSelected ? "2px solid #3B82F6" : "1px solid #E2E8F0", borderRadius: 16, padding: "20px 24px", cursor: "pointer", transition: "all 0.2s", position: "relative", overflow: "hidden", boxShadow: isSelected ? "0 4px 16px rgba(59,130,246,0.1)" : "0 2px 8px rgba(0,0,0,0.02)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    
                    <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                      <div style={{ width: 48, height: 48, borderRadius: 12, background: isSelected ? "#DBEAFE" : "#F1F5F9", color: isSelected ? "#1D4ED8" : "#64748B", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Package size={24} />
                      </div>
                      <div>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                          <span style={{ fontWeight: 900, fontSize: 18, color: "#0F172A", fontFamily: "monospace" }}>{b.kode_batch}</span>
                          <span style={{ padding: "4px 10px", borderRadius: 99, fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: 0.5, background: b.route_selesai ? "#D1FAE5" : "#FEF3C7", color: b.route_selesai ? "#065F46" : "#92400E" }}>
                            {b.route_selesai ? "Selesai" : "Proses"}
                          </span>
                          {batchPernahRetur(b.id) && <span style={{ padding: "4px 8px", background: "#FEE2E2", color: "#B91C1C", borderRadius: 6, fontSize: 10, fontWeight: 900 }}>RETUR</span>}
                        </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 24, fontWeight: 900, color: "#10B981", lineHeight: 1 }}>{final} <span style={{fontSize:14,color:"#64748B",fontWeight:700}}>pcs</span></div>
                        <div style={{ fontSize: 12, color: "#64748B", marginTop: 6, fontWeight: 600 }}>{cacat > 0 ? <span style={{color:"#EF4444"}}>{cacat} Cacat</span> : "Valid"}</div>
                      </div>
                      <ChevronRight size={24} color={isSelected ? "#3B82F6" : "#CBD5E1"} />
                    </div>
        ...
        {selected && (
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, zIndex: 100, display: "flex", justifyContent: "flex-end", overflow: "hidden" }}>
            ...
            <div style={{ width: 440, background: "white", height: "100%", boxShadow: "-10px 0 30px rgba(0,0,0,0.1)", position: "relative", zIndex: 101, display: "flex", flexDirection: "column", animation: "slideIn 0.3s cubic-bezier(0.16, 1, 0.3, 1)" }}>

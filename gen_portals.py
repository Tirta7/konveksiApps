import os, sys
paths = [
    ('src/app/washing/[id]/page.tsx', 'Washing', 'washing', '#06B6D4', 'Droplets', 'Droplets'),
    ('src/app/bersih-benang/[id]/page.tsx', 'Bersih Benang', 'benang', '#F59E0B', 'Scissors', 'Scissors'),
]
for path, label, ke_field, color, icon_import, icon_use in paths:
    d = os.path.dirname(path)
    os.makedirs(d, exist_ok=True)
    content = '''\
"use client";
import React, { use, useEffect, useState, useCallback } from "react";
import { ''' + icon_import + ''', X } from "lucide-react";
...
export default function PortalPage({ params }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [resT, resV] = await Promise.all([
        fetch("/api/produksi-transfer?ke=''' + ke_field + '''&_t=" + Date.now()),
  ...
  const submitLapor = async (e) => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/produksi-transfer", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({po_id:showModal.po_id,dari:"''' + ke_field + '''",ke:"gudang",vendor_id:id,jumlah_kirim:total,sizeBreakdown:items,catatan}) });
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0F172A"}}>
      <div style={{background:"#1E293B",padding:"24px 28px",borderBottom:"1px solid #334155",display:"flex",alignItems:"center",gap:16}}>
        <div style={{background:"''' + color + '''",width:48,height:48,borderRadius:14,display:"flex",alignItems:"center",justifyContent:"center"}}>
          <''' + icon_use + ''' size={24} color="white" />
        </div>
        <div>
          <h1 style={{margin:0,color:"white",fontSize:20,fontWeight:800}}>Portal ''' + label + ''' &mdash; {vendor?.nama||"..."}</h1>
      ...
      <div style={{padding:28}}>
        {aktif.length===0 ? (
          <div style={{background:"#1E293B",borderRadius:16,padding:40,textAlign:"center"}}>
            <div style={{color:"#64748B",fontWeight:700}}>Belum ada pakaian yang dikirim ke tahap ''' + label + '''</div>
          </div>
        ) : aktif.map(t => (
          <div key={t.id} style={{background:"#1E293B",borderRadius:16,padding:20,marginBottom:12,border:"1px solid #334155"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:16,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>openLapor(t)} style={{background:"''' + color + '''",color:"white",padding:"10px 18px",borderRadius:10,border:"none",fontWeight:800,cursor:"pointer",fontSize:13}}>Lapor Selesai</button>
            </div>
          </div>
        ))}
      </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:100,padding:20}}>
          <div style={{background:"#1E293B",borderRadius:20,width:"100%",maxWidth:480,border:"1px solid #334155"}}>
            ...
            <form onSubmit={submitLapor} style={{padding:24}}>
              ...
              <button type="submit" disabled={submitting} style={{width:"100%",background:"''' + color + '''",color:"white",padding:15,borderRadius:12,border:"none",fontWeight:800,fontSize:15,cursor:"pointer"}}>{submitting?"Mengirim...":"Kirim Laporan ke Gudang"}</button>
            </form>
          </div>
        </div>
      )}
